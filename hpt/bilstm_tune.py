...
def build_bilstm_model(input_shape, num_layers, neurons, activation, dropout, learning_rate, loss_function):
    model = Sequential()
    for i in range(num_layers):
        return_seq = i < num_layers - 1
        if i == 0:
            model.add(Bidirectional(LSTM(neurons, activation=activation, return_sequences=return_seq), input_shape=input_shape))
        else:
            model.add(Bidirectional(LSTM(neurons, activation=activation, return_sequences=return_seq)))
...
def run_architectural_tuning(train_df, dev_df, batch_ids):
    ...
    for gid in batch_ids:
        ...
        for num_layers, neurons in product(ARCHITECTURAL_PARAMS["num_layers"], ARCHITECTURAL_PARAMS["neurons"]):
            ...
            y_train_seq = y_train[TRAINING_PARAMS["lookback"]:TRAINING_PARAMS["lookback"] + len(X_train_seq)]
            X_dev_seq = reshape_for_sequence(X_dev, TRAINING_PARAMS["lookback"])
            y_dev_seq = y_dev[TRAINING_PARAMS["lookback"]:TRAINING_PARAMS["lookback"] + len(X_dev_seq)]
...
def run_training_param_tuning(train_df, dev_df, batch_ids, param, values):
    results = []
    for gid in batch_ids:
        train_site = train_df[train_df["gauge_seq_id"] == gid].copy()
        dev_site = dev_df[dev_df["gauge_seq_id"] == gid].copy()
        ...
        for value in values:
            ...
            X_train_seq = reshape_for_sequence(X_train, temp_params["lookback"])
            y_train_seq = y_train[temp_params["lookback"]:temp_params["lookback"] + len(X_train_seq)]
            X_dev_seq = reshape_for_sequence(X_dev, temp_params["lookback"])
            y_dev_seq = y_dev[temp_params["lookback"]:temp_params["lookback"] + len(X_dev_seq)]
            ...
            model = build_bilstm_model(
                (X_train_seq.shape[1], X_train_seq.shape[2]),
                2,  # Default architecture for training param tuning
                64,
                temp_params["activation"],
                temp_params["dropout"],
                temp_params["learning_rate"],
                temp_params["loss_function"]
            )
    ...
    start_seq = batch_ids[0]
    batch_size = len(batch_ids)
    results_df.to_csv(os.path.join(RESULTS_DIR, f"bilstm_training_hpt_{param}_batch_{start_seq}_to_{start_seq + batch_size - 1}.csv"), index=False)
