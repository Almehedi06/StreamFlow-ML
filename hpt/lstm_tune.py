...
def build_lstm_model(input_shape, num_layers, neurons):
    model = Sequential()
...
def run_architectural_tuning(train_df, dev_df, batch_ids):
    ...
    for gid in batch_ids:
        ...
        for num_layers, neurons in product(ARCHITECTURAL_PARAMS["num_layers"], ARCHITECTURAL_PARAMS["neurons"]):
            ...
            model = build_lstm_model((X_train_seq.shape[1], X_train_seq.shape[2]), num_layers, neurons)
    ...
    results_df.to_csv(os.path.join(RESULTS_DIR, f"LSTM_HPT_results_batch_{start_seq}_to_{start_seq + batch_size - 1}.csv"), index=False)
...
def run_training_param_tuning(train_df, dev_df, batch_ids, tune_param, tune_values):
    ...
    for gid in batch_ids:
        ...
        for value in tune_values:
            ...
            model = build_lstm_model((X_train_seq.shape[1], X_train_seq.shape[2]), 2, 64)
