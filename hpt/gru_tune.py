...
def build_gru_model(input_shape, num_layers, neurons):
    model = Sequential()
    for i in range(num_layers):
        return_seq = i < num_layers - 1
        if i == 0:
            model.add(GRU(neurons, activation=TRAINING_PARAMS["activation"], return_sequences=return_seq, input_shape=input_shape))
        else:
            model.add(GRU(neurons, activation=TRAINING_PARAMS["activation"], return_sequences=return_seq))
...
def run_architectural_tuning(train_df, dev_df, batch_ids):
    ...
    for gid in batch_ids:
        ...
        for num_layers, neurons in product(ARCHITECTURAL_PARAMS["num_layers"], ARCHITECTURAL_PARAMS["neurons"]):
            ...
            model = build_gru_model((X_train_seq.shape[1], X_train_seq.shape[2]), num_layers, neurons)
    ...
    df.to_csv(os.path.join(RESULTS_DIR, f"GRU_HPT_results_batch_{start_seq}_to_{start_seq + batch_size - 1}.csv"), index=False)
...
def run_training_param_tuning(train_df, dev_df, batch_ids, tune_param, tune_values):
    ...
    for gid in batch_ids:
        ...
        for value in tune_values:
            ...
            model = build_gru_model((X_train_seq.shape[1], X_train_seq.shape[2]), 2, 64)
    ...
    start_seq = batch_ids[0]
    batch_size = len(batch_ids)
    df.to_csv(os.path.join(RESULTS_DIR, f"GRU_HPT_{tune_param}_batch_{start_seq}_to_{start_seq + batch_size - 1}.csv"), index=False)
